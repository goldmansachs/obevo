...
public class DbFileMerger {
    private static final Logger LOG = LoggerFactory.getLogger(DbFileMerger.class);
...
//            addDistinctValue(normalizedContent);
        ...
        @Override
        public int hashCode() {
            return Objects.hash(name, changeType);
        }
    ...
    private void generateDiffs(Platform dialect, RichIterable<DbMergeInfo> dbNameLocationPairs, File outputDir) {
        LOG.info("Generating diffs for {}", dbNameLocationPairs);
        ...
        for (DbMergeInfo dbNameLocationPair : dbNameLocationPairs) {
            ...
            for (final FileObject schemaDir : mainDir.getChildren()) {
                ...
                for (final ChangeType changeType : dialect.getChangeTypes()) {
                    LOG.info("Checking change type {} in schema dir {}", changeType.getName(), schemaDir);
