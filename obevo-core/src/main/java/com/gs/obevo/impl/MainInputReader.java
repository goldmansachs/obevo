...
public class MainInputReader<P extends Platform, E extends Environment<P>> {
    ...
    private void logChangeMetrics(final String changeSide, ImmutableList<Change> changes) {
        MutableBag<String> changeTypeCounts = changes.collect(new Function<Change, String>() {
            @Override
    ...
    private ImmutableList<Change> readSourceChanges(SourceReaderStrategy dbChangeReader, boolean useBaseline, Predicate<Change> dbChangeFilter) {
        ...
        CollectionUtil.verifyNoDuplicates(sourceChanges, new Function<Change, Object>() {
            @Override
            public Object valueOf(Change change1) {
                return change1.getChangeKey();
            }
        }, "Duplicate changes found - please check your input source files (e.g. no //// CHANGE entries with the same name in a file or files w/ same object names within an environment)");
