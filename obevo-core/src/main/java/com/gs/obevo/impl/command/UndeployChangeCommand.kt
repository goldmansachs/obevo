...
class UndeployChangeCommand(artifact: Change, undeployMessage: String) : AbstractExecuteChangeCommand(artifact, undeployMessage) {
    override fun execute(changeTypeBehaviorRegistry: ChangeTypeBehaviorRegistry, cec: CommandExecutionContext) {
        changeTypeBehaviorRegistry.undeploy(this.artifact)
    }
