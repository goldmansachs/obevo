...
public class MySqlDumpReveng extends AbstractDdlReveng {
    ...
    private static ImmutableList<RevengPattern> getRevengPatterns() {
        ...
        return Lists.immutable.with(
                ...
                new RevengPattern(ChangeType.VIEW_STR, namePatternType, "(?i)/*!\\d+\\s+create\\s+(?:or\\s+replace\\s+)?(?:force\\s+)?(?:editionable\\s+)?view\\s+" + schemaNameSubPattern).withKeepLastOnly(true),
    ...
    protected boolean doRevengOrInstructions(PrintStream out, AquaRevengArgs args, File interimDir) {
        ...
        out.println();
        out.println(getCommandWithDefaults(args, interimDir, "<username>", "<password>", "<dbHost>", "<dbPortNumber>", "<dbName>", "<dbSchema>", "<outputFile>"));
        out.println();
        out.println("Here is an example command (in case your values are not filled in):");
        out.println();
        out.println(getCommandWithDefaults(args, interimDir, "myuser", "mypassword", "myhost.myplace.com", "12345", "mydb", "myschema", "H:\\ddl-output.txt"));
        out.println();

        return false;
    }

    private String getCommandWithDefaults(AquaRevengArgs args, File interimDir, String username, String password, String dbHost, String dbPort, String dbName, String dbSchema, String outputDirectory) {
        return "mysqldump" +
                " -u " + ObjectUtils.defaultIfNull(args.getUsername(), username) +
                " --password=" + ObjectUtils.defaultIfNull(args.getPassword(), password) +
                " --databases " + ObjectUtils.defaultIfNull(args.getDbSchema(), dbSchema) +
                " --compact --compatible=ANSI --no-create-db --skip-quote-names --skip-dump-date --no-data --skip-add-drop-table" +
                " > " + new File(interimDir, "revengoutput.txt").getAbsolutePath();
    }
}
