CREATE PROCEDURE SP_REMOVE_PROCESS(
            IN in_process_name varchar(100),
            IN in_deployment_id INTEGER,
            IN in_published varchar(1),
...
BEGIN
	...
	begin
		...
		begin
	            ...
	            if (in_process_name is null or in_deployment_id is null or in_published is null) then
					...
					update T_PROCESS set modified_by=in_modified_by,modified_on=current timestamp where PROCESS_NAME=in_process_name and DEPLOYMENT_ID=in_deployment_id and PUBLISHED=in_published;
					delete from T_PROCESS where PROCESS_NAME=in_process_name and DEPLOYMENT_ID=in_deployment_id and PUBLISHED=in_published;
                end if;
		end;
    end; 
END
