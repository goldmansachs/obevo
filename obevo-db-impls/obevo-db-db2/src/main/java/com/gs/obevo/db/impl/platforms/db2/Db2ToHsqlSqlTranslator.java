...
public class Db2ToHsqlSqlTranslator implements PostColumnSqlTranslator {
    @Override
    public String handlePostColumnText(String sql, CreateTableColumn column, CreateTable table) {
        ...
        if (identityMatcher.find()) {
            return identityMatcher.replaceFirst("generated " + identityMatcher.group(1) + " as identity ");
        } else {
            return sql;
        }
    }
